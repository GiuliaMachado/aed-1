...
float list_averageGrade(StudentList* list){
    ...
    media = media / list_size(list);

    return media;
}

StudentList* list_filterFailed(StudentList* list){
    StudentList* new_list = list_create();
    Node* cur = list->head;

    while(cur != NULL ){
        if(cur->student->grade < 6.00){
            list_insertFirst(new_list, cur->student->ra, cur->student->name, cur->student->grade);
        }
         cur = cur->next;
    }
    return new_list;
...
StudentList* list_filterExam(StudentList* list){
    ...
    while(cur != NULL){
        if(cur->student->grade >= 4.00 && cur->student->grade < 6.00){
            list_insertFirst(new_list, cur->student->ra, cur->student->name, cur->student->grade);
        }
        cur = cur->next;
    }
